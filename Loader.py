...
def args(mode):
    ...
    parser.add_argument('--mhard', default=4, help='hard example factor')
    parser.add_argument('--blambda', default=1, help='weight to balance saliency and contour losses')
    parser.add_argument('--kernel', default=5, help='kernel size for morphological operation')
    #parser.add_argument('--evalparam', action='store_true')
...
class Loader(object):
    ...
    def loading(self, opt):
        ...
        if self.MODE == 'train':
            ...
            self.lr = opt.lr
            self.mhard = opt.mhard
            self.blambda = opt.blambda

            self.plist = [['encoder', self.lr*0.1], ['decoder', self.lr]]

            self.trset = 'SOD' if self.debug else opt.trset
            self.trSet = src.DataSet(self.trset, mode='train', shape=opt.size, debug=self.debug, kernel_sz=opt.kernel)
        ...
        print('Image size: {}'.format(opt.size))
