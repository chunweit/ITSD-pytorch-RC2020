...
class PRF(nn.Module):
    ...
    def forward(self, _Y):
        TPs = [torch.sum(torch.sum((_Y >= threshold) & (self.Y.bool()), -1), -1).float() for threshold in self.thresholds]
...
def getOutPuts(model, DX, args, supervised=False):
    ...
    for idx in range(0, len(DX), args.batch):
        ...
        t_inf = time.time()
        Outs = model(X)
        inf_times.append(time.time() - t_inf)
    ...
    OutPuts['avg_inf_time'] = float(inf_times.mean())
    #print('>> {}\tavg inference time: {:.2f}s'.format())
