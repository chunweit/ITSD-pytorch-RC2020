...
IMG_EXTS = ['.jpg', '.jpeg', '.tif', '.tiff', '.bmp', '.png']
...
class _DataSet(object):
    ...
    def getNames(self):
        names = list(map(lambda x : sorted([f for f in os.listdir(x) if os.path.splitext(f)[-1] in IMG_EXTS]), self.path))
...
class DataSet(_DataSet):
    def __init__(self, name, mode='train', shape=224, debug=False, kernel_sz=5):
    ...
    def getFull(self):
        ...
        if self.debug:
            ...
            kernel = np.ones((self.kernel_sz, self.kernel_sz))
            for idx, y in enumerate(DataDict['Y']):
                DataDict['C'][idx] = y#cv2.dilate(y, kernel) - cv2.erode(y, kernel)
