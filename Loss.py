...
def ACT(X, batchs, args):
    ...
    _m = int(args.mhard)
    _lambda = float(args.blambda)
    #print('m hard: ', _m)
    #print('labmda: ', _lambda)
    ...
    for slc_pred, ctr_pred, weight in zip(X['preds'], X['contour'], args.weights):
        ...
        # ACT loss
        #print('act: weight: ', weight)
        #print()
        pc = ctr_pred.sigmoid_()
        w = torch.where(pc > yc, pc, yc)
        #print('>>> type of m: ', type(_m))

        slc_loss += (bce(slc_pred, ys) * (w * _m + 1)).mean() * weight
    ...
    #print('>>> arg weights: ', args.weights[-1])
    fnl_loss = (bce(X['final'], slc_gt.gt(0.5).float()) * (w * _m + 1)).mean() * args.weights[-1]
    #print('fnl_loss shape: ', fnl_loss, ctr_loss, slc_loss)

    return fnl_loss + (_lambda*ctr_loss + slc_loss)
